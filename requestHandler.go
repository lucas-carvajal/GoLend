...
func handleSignup(
	...
	case response.Status > 399:
		sendErrorResponse(w, response.Message, response.Status)
	case response.Status < 210:
		sendSuccessResponse(w, response.Status)
	}
}
...
func handleLogin(
	...
	case response.Status > 399:
		sendErrorResponse(w, response.Message, response.Status)
	case response.Status < 210:
		sendSuccessResponseWithToken(w, response.Status, response.Token)
	}
}
